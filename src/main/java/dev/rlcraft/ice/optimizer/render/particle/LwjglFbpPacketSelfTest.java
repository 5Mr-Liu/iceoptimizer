...
/** Executable output test for the exact fixed-function FBP BLOCK packet path. */
public final class LwjglFbpPacketSelfTest {
    private static final int MAX_FIXED_FUNCTION_TEXTURE_UNITS = 32;

    private LwjglFbpPacketSelfTest() {
    }
    ...
    public static Result validate(CacheBudget budget) {
        ...
        if (capabilities == null || !capabilities.OpenGL15
            ...
            return new Result(false, "FBP VBO/VAO/FBO prerequisites unavailable");
        }
        if (budget == null) {
            return new Result(false, "FBP self-test GPU budget unavailable");
        }
        ...
        TemporaryGpuResourceScope.Slot bufferSlot = vertexArraySlot == null
            ? null : scratch.reserve(RenderResourceKind.BUFFER,
                LwjglFbpPacketRenderer.BLOCK_STRIDE * 4L,
                LwjglTemporaryResourceOps.DELETE_BUFFER);
        if (bufferSlot == null) {
            Throwable cleanup = scratch.closeAndAppend(null);
            return cleanup == null
                ? new Result(false, "FBP self-test GPU budget exhausted")
                : failure(cleanup);
        }
        ...
        try {
            ...
            if (framebuffer <= 0 || texture <= 0) {
                throw new IllegalStateException(
                    "FBP self-test framebuffer object allocation failed");
            }
            ...
            if (GL30.glCheckFramebufferStatus(GL30.GL_FRAMEBUFFER)
                ...
                throw new IllegalStateException("FBP self-test FBO incomplete");
            }
            ...
            if (vertexArray <= 0 || buffer <= 0) {
                throw new IllegalStateException("FBP self-test object allocation failed");
            }
            GL30.glBindVertexArray(vertexArray);
            GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, buffer);
            ByteBuffer vertices = packet();
            GL15.glBufferData(GL15.GL_ARRAY_BUFFER, vertices, GL15.GL_STREAM_DRAW);
            LwjglFbpPacketRenderer.setupBlockArrays();
            GL11.glDrawArrays(GL11.GL_QUADS, 0, 4);

            ByteBuffer pixel = BufferUtils.createByteBuffer(4);
            GL11.glReadPixels(2, 2, 1, 1, GL11.GL_RGBA,
                GL11.GL_UNSIGNED_BYTE, pixel);
            requireNear(pixel.get(0) & 255, 37, "red");
            requireNear(pixel.get(1) & 255, 149, "green");
            requireNear(pixel.get(2) & 255, 211, "blue");
            requireNear(pixel.get(3) & 255, 255, "alpha");
            passed = true;
        ...
        return passed && failure == null
            ? new Result(true,
                "exact 28-byte BLOCK VBO/client-array output verified")
            : failure(failure);
    }

    private static ByteBuffer packet() {
        ByteBuffer data = BufferUtils.createByteBuffer(
            LwjglFbpPacketRenderer.BLOCK_STRIDE * 4).order(ByteOrder.nativeOrder());
        vertex(data, -1.0F, -1.0F, 0.0F, 0.0F);
        vertex(data, 1.0F, -1.0F, 1.0F, 0.0F);
        vertex(data, 1.0F, 1.0F, 1.0F, 1.0F);
        vertex(data, -1.0F, 1.0F, 0.0F, 1.0F);
        data.flip();
        return data;
    }

    private static void vertex(ByteBuffer data, float x, float y,
                               float u, float v) {
        data.putFloat(x).putFloat(y).putFloat(0.0F);
        data.put((byte) 37).put((byte) 149).put((byte) 211).put((byte) 255);
        data.putFloat(u).putFloat(v);
        data.putShort((short) 240).putShort((short) 240);
    }

    private static void requireNear(int actual, int expected, String component) {
        if (Math.abs(actual - expected) > 3) {
            throw new IllegalStateException("FBP " + component
    ...
    private static void disableFixedFunctionTextures() {
        ...
        if (units < 1 || units > MAX_FIXED_FUNCTION_TEXTURE_UNITS) {
            throw new IllegalStateException(
                "FBP fixed-function texture-unit count outside probe limit: "
                    + units);
        }
    ...
    private static Result failure(Throwable error) {
        ...
        String message = error == null ? "unknown"
            : error.getClass().getSimpleName() + (error.getMessage() == null
                ? "" : ": " + error.getMessage());
        return new Result(false, message.length() <= 160
