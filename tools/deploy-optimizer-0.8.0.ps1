...
$sourceMain = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-0.8.0.jar'
$sourceCore = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-core-0.8.0.jar'
$expectedMainHash = 'E4AD99EBB23E31E40D6F80BF3BAE76B63BDDD8F58691B1E7612178D884D0BF3F'
$expectedCoreHash = '2E08FB46CB7C8C207FD61AC96EB7A4D61B1C21B575480A2CCC61157E3078028A'
...
if ($javaProcesses.Count -ne 0) {
    throw 'Minecraft or the dedicated server is still running. Stop every Java game process before deploying ICE Optimizer 0.8.0.'
}
...
$newMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.8.0.jar'
$newCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.8.0.jar'
$stageMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.8.0.jar.deploying'
$stageCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.8.0.jar.deploying'
...
$unexpectedCurrent = @($installed | Where-Object {
    $_.Name -notin @('ice-rlcraft-optimizer-0.8.0.jar', 'ice-rlcraft-optimizer-core-0.8.0.jar')
})
$alreadyCurrent =
...
if ($alreadyCurrent) {
    ...
    Write-Host "ICE RLCraft Optimizer 0.8.0 is already deployed and verified for $Pack $Target." -ForegroundColor Green
    exit 0
}

$timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupDirectory = Join-Path $projectRoot "rollback\optimizer-$Pack-$Target-before-0.8.0-$timestamp"
...
Write-Host "ICE RLCraft Optimizer 0.8.0 deployed successfully for $Pack $Target." -ForegroundColor Green
