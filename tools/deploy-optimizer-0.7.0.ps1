...
$sourceMain = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-0.7.0.jar'
$sourceCore = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-core-0.7.0.jar'
$expectedMainHash = 'A8246461DBD5E28CD6C5BCE394C1F65B98BC38E13274A3D35391916A1DF679D5'
$expectedCoreHash = '56EA8E078849BA09316173302F6027F6B5AC80B73DB6C647B80FA754F999754C'
...
if ($javaProcesses.Count -ne 0) {
    throw 'Minecraft or the dedicated server is still running. Stop every Java game process before deploying ICE Optimizer 0.7.0.'
}
...
$newMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.7.0.jar'
$newCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.7.0.jar'
$stageMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.7.0.jar.deploying'
$stageCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.7.0.jar.deploying'
...
$unexpectedCurrent = @($installed | Where-Object {
    $_.Name -notin @('ice-rlcraft-optimizer-0.7.0.jar', 'ice-rlcraft-optimizer-core-0.7.0.jar')
})
$alreadyCurrent =
...
if ($alreadyCurrent) {
    ...
    Write-Host "ICE RLCraft Optimizer 0.7.0 is already deployed and verified for $Pack $Target." -ForegroundColor Green
    exit 0
}

$timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupDirectory = Join-Path $projectRoot "rollback\optimizer-$Pack-$Target-before-0.7.0-$timestamp"
...
Write-Host "ICE RLCraft Optimizer 0.7.0 deployed successfully for $Pack $Target." -ForegroundColor Green
