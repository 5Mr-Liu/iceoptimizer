...
$sourceMain = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-0.9.1.jar'
$sourceCore = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-core-0.9.1.jar'
$expectedMainHash = 'E3056C6B01AA88979A73B0446FBD33304CC327EDBB7E8AE8E948DDA4BA045BC4'
$expectedCoreHash = 'FCCBB59023B5435B82370D74ED84558A4E673DDE96BB489088B7897B2B8285FA'
...
if ($javaProcesses.Count -ne 0) {
    throw 'Minecraft or the dedicated server is still running. Stop every Java game process before deploying ICE Optimizer 0.9.1.'
}
...
$newMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.9.1.jar'
$newCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.9.1.jar'
$stageMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.9.1.jar.deploying'
$stageCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.9.1.jar.deploying'
...
$unexpectedCurrent = @($installed | Where-Object {
    $_.Name -notin @('ice-rlcraft-optimizer-0.9.1.jar', 'ice-rlcraft-optimizer-core-0.9.1.jar')
})
$alreadyCurrent =
...
if ($alreadyCurrent) {
    ...
    Write-Host "ICE RLCraft Optimizer 0.9.1 is already deployed and verified for $Pack $Target." -ForegroundColor Green
    exit 0
}

$timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupDirectory = Join-Path $projectRoot "rollback\optimizer-$Pack-$Target-before-0.9.1-$timestamp"
...
Write-Host "ICE RLCraft Optimizer 0.9.1 deployed successfully for $Pack $Target." -ForegroundColor Green
