...
$sourceMain = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-0.9.4.jar'
$sourceCore = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-core-0.9.4.jar'
$expectedMainHash = 'AD222C7BFEE862F6C0233E2A77E7F2B4C497C500877AB668570DA570EDADACA9'
$expectedCoreHash = 'C36DB8D2B20BB8235E3994966106B6AFACD47E09A02FBC46A35F343AC075B164'
...
if ($javaProcesses.Count -ne 0) {
    throw 'Minecraft or the dedicated server is still running. Stop every Java game process before deploying ICE Optimizer 0.9.4.'
}
...
$newMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.9.4.jar'
$newCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.9.4.jar'
$stageMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.9.4.jar.deploying'
$stageCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.9.4.jar.deploying'
...
$unexpectedCurrent = @($installed | Where-Object {
    $_.Name -notin @('ice-rlcraft-optimizer-0.9.4.jar', 'ice-rlcraft-optimizer-core-0.9.4.jar')
})
$alreadyCurrent =
...
if ($alreadyCurrent) {
    ...
    Write-Host "ICE RLCraft Optimizer 0.9.4 is already deployed and verified for $Pack $Target." -ForegroundColor Green
    exit 0
}

$timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupDirectory = Join-Path $projectRoot "rollback\optimizer-$Pack-$Target-before-0.9.4-$timestamp"
...
Write-Host "ICE RLCraft Optimizer 0.9.4 deployed successfully for $Pack $Target." -ForegroundColor Green
