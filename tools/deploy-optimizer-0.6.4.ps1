...
$sourceMain = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-0.6.4.jar'
$sourceCore = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-core-0.6.4.jar'
$expectedMainHash = '49A76ACCBD5495F670C92F5722E42469DD805342475A470461E78B54CBB91F21'
$expectedCoreHash = '332EC4D4E4F00EB95791AB9D26282E35A9E1B39872653F37EE3F99D8E30EAD38'
...
if ($javaProcesses.Count -ne 0) {
    throw 'Minecraft/Java is still running. Close it before deploying ICE Optimizer 0.6.4.'
}
...
$newMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.6.4.jar'
$newCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.6.4.jar'
$stageMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.6.4.jar.deploying'
$stageCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.6.4.jar.deploying'
...
$unexpectedCurrent = @($installed | Where-Object {
    $_.Name -notin @('ice-rlcraft-optimizer-0.6.4.jar', 'ice-rlcraft-optimizer-core-0.6.4.jar')
})
$alreadyCurrent =
...
if ($alreadyCurrent) {
    ...
    Write-Host "ICE RLCraft Optimizer 0.6.4 is already deployed and verified for $Pack." -ForegroundColor Green
    exit 0
}

$timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupDirectory = Join-Path $projectRoot "rollback\optimizer-$Pack-before-0.6.4-$timestamp"
...
Write-Host "ICE RLCraft Optimizer 0.6.4 deployed successfully for $Pack." -ForegroundColor Green
