...
$sourceMain = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-0.6.3.jar'
$sourceCore = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-core-0.6.3.jar'
$expectedMainHash = '3EEE00FC5CEFC171E4CE941F279462F9DC36ECB5750C2F872BE7B6CA3F40C142'
$expectedCoreHash = 'F33C220D1616A2596219CADF569673C6079D05145CE5450FEC82D0AAB968539C'
...
if ($javaProcesses.Count -ne 0) {
    throw 'Minecraft/Java is still running. Close it before deploying ICE Optimizer 0.6.3.'
}
...
$newMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.6.3.jar'
$newCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.6.3.jar'
$stageMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.6.3.jar.deploying'
$stageCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.6.3.jar.deploying'
...
$unexpectedCurrent = @($installed | Where-Object {
    $_.Name -notin @('ice-rlcraft-optimizer-0.6.3.jar', 'ice-rlcraft-optimizer-core-0.6.3.jar')
})
$alreadyCurrent =
...
if ($alreadyCurrent) {
    ...
    Write-Host "ICE RLCraft Optimizer 0.6.3 is already deployed and verified for $Pack." -ForegroundColor Green
    exit 0
}

$timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupDirectory = Join-Path $projectRoot "rollback\optimizer-$Pack-before-0.6.3-$timestamp"
...
Write-Host "ICE RLCraft Optimizer 0.6.3 deployed successfully for $Pack." -ForegroundColor Green
