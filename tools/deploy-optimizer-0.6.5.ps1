...
$sourceMain = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-0.6.5.jar'
$sourceCore = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-core-0.6.5.jar'
$expectedMainHash = '524FE47E387C1436605FC9BF33FC811DF7C73C0C832094DACC6B78C3CA8AFF8E'
$expectedCoreHash = 'A4F55A0A872050A230BA3162D2EA8DD5FB749F0C89FC89E781BECCC99F49C25D'
...
if ($javaProcesses.Count -ne 0) {
    throw 'Minecraft/Java is still running. Close it before deploying ICE Optimizer 0.6.5.'
}
...
$newMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.6.5.jar'
$newCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.6.5.jar'
$stageMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.6.5.jar.deploying'
$stageCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.6.5.jar.deploying'
...
$unexpectedCurrent = @($installed | Where-Object {
    $_.Name -notin @('ice-rlcraft-optimizer-0.6.5.jar', 'ice-rlcraft-optimizer-core-0.6.5.jar')
})
$alreadyCurrent =
...
if ($alreadyCurrent) {
    ...
    Write-Host "ICE RLCraft Optimizer 0.6.5 is already deployed and verified for $Pack." -ForegroundColor Green
    exit 0
}

$timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupDirectory = Join-Path $projectRoot "rollback\optimizer-$Pack-before-0.6.5-$timestamp"
...
Write-Host "ICE RLCraft Optimizer 0.6.5 deployed successfully for $Pack." -ForegroundColor Green
