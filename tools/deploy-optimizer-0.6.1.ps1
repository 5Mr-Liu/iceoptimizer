...
$sourceMain = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-0.6.1.jar'
$sourceCore = Join-Path $projectRoot 'build\libs\ice-rlcraft-optimizer-core-0.6.1.jar'
$expectedMainHash = 'A7CA78524A089315860EC6FA4AFE3DABEC9DED4439AA8FF4B6E0196552D1E1C8'
$expectedCoreHash = '0564B7351D7F9598B8F5318D1E7882D4C6B8E79AFD48AAFB07FCB4B96BEE7958'
...
if ($javaProcesses.Count -ne 0) {
    throw 'Minecraft/Java is still running. Close it before deploying ICE Optimizer 0.6.1.'
}
...
$newMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.6.1.jar'
$newCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.6.1.jar'
$stageMain = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-0.6.1.jar.deploying'
$stageCore = Join-Path $ModsDirectory 'ice-rlcraft-optimizer-core-0.6.1.jar.deploying'
...
$unexpectedCurrent = @($installed | Where-Object {
    $_.Name -notin @('ice-rlcraft-optimizer-0.6.1.jar', 'ice-rlcraft-optimizer-core-0.6.1.jar')
})
$alreadyCurrent =
...
if ($alreadyCurrent) {
    ...
    Write-Host "ICE RLCraft Optimizer 0.6.1 is already deployed and verified for $Pack." -ForegroundColor Green
    exit 0
}

$timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupDirectory = Join-Path $projectRoot "rollback\optimizer-$Pack-before-0.6.1-$timestamp"
...
Write-Host "ICE RLCraft Optimizer 0.6.1 deployed successfully for $Pack." -ForegroundColor Green
